...
function attrsObject(selection$$1, map) {
  for (let name in map) selection$$1.attr(name, map[name]);
  return selection$$1;
}

let selection_attrs = function(map) {
...
function stylesObject(selection$$1, map, priority) {
  for (let name in map) selection$$1.style(name, map[name], priority);
  return selection$$1;
}

let selection_styles = function(map, priority) {
...
function propertiesObject(selection$$1, map) {
  for (let name in map) selection$$1.property(name, map[name]);
  return selection$$1;
}

let selection_properties = function(map) {
  return (typeof map === "function" ? propertiesFunction : propertiesObject)(this, map);
};

function attrsFunction$1(transition$$1, map) {
  return transition$$1.each(function() {
    let x = map.apply(this, arguments), t = d3Selection.select(this).transition(transition$$1);
    for (let name in x) t.attr(name, x[name]);
  });
}

function attrsObject$1(transition$$1, map) {
  for (let name in map) transition$$1.attr(name, map[name]);
  return transition$$1;
}

let transition_attrs = function(map) {
...
function stylesObject$1(transition$$1, map, priority) {
  for (let name in map) transition$$1.style(name, map[name], priority);
  return transition$$1;
}
